...
def create_app() -> FastAPI:
    ...
    async def get_default_config():
        ...
        The response also includes the CLI-specified model and server URL so
        the frontend can use them as defaults.
        """
    ...
    async def all_models():
        """Return the list of actually-available models from the LLM proxy.

        Forwards the proxy's ``/models`` response, which only includes
        routes whose API keys were found at proxy startup (i.e. option B:
        display models that the system can actually route right now).

        If the proxy is unreachable the error propagates so the frontend
        knows to fall back to its built-in list.
        """
        server_url = getattr(
            app.state, "default_server_url",
            "http://127.0.0.1:8110/chat/completions",
        )
        base = server_url.rsplit("/chat/completions", 1)[0]
        async with httpx.AsyncClient() as client:
            try:
                resp = await client.get(f"{base}/models", timeout=5)
                resp.raise_for_status()
                return resp.json()
            except httpx.HTTPError as exc:
                raise HTTPException(
                    status_code=502,
                    detail=f"LLM proxy unreachable at {base}/models: {exc}",
                ) from exc
