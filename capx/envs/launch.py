...
class LaunchArgs:
    ...
    """Experiment campaign name. If set, inserted as a top-level directory under outputs/ (before the model name)."""

    timestamp: bool = False
    """If True, append a timestamp subdirectory to the output path (useful for debugging to keep a fixed path)."""
    ...
    """If False (default), skip the run when the output directory already contains
    >= total_trials completed ``trial_*`` sub-dirs (or the ``aaa_done_flag`` is
    present). If True, always run and let trials overwrite existing artifacts."""


def _is_run_complete(out_dir: str, total_trials: int) -> tuple[bool, int]:
    """Return (is_complete, n_completed_trials) for an existing output dir.

    Considered complete when the run finished flag is present, or when the
    number of ``trial_*`` sub-directories already reaches ``total_trials``.
    """
    p = Path(out_dir)
    if not p.exists():
        return False, 0
    if (p / "aaa_done_flag" / "aaa_done_flag.txt").exists():
        return True, total_trials
    n = sum(1 for child in p.glob("trial_*") if child.is_dir())
    return n >= total_trials, n
...
def main(args: LaunchArgs) -> None:
    ...
    # Resolve output dir up-front (must happen exactly once: it splices the
    # model_tag into config["output_dir"]) so we can probe completion before
    # spinning up API servers. Skip in web_ui mode where there is no fixed
    # trial budget.
    is_web_ui = bool(config.get("web_ui", False))
    if not is_web_ui:
        # _setup_output_dir is a no-op when config["output_dir"] is falsy,
        # which is the legitimate "run without persisting artifacts" mode.
        # In that mode we have nothing on disk to compare against, so the
        # overwrite/skip check is also bypassed.
        _setup_output_dir(args, config)
        if config["output_dir"]:
            done, n_done = _is_run_complete(
                config["output_dir"], config["total_trials"]
            )
            if done and not args.overwrite:
                print(
                    f"[skip] {config['output_dir']} already has {n_done}/"
                    f"{config['total_trials']} trials. Pass --overwrite to force rerun."
                )
                return
