...
if _libero_available:
    ...
    # Default: SAM3 only. No dependency on the Molmo vLLM service (port 8122).
    register_api(
        "FrankaLiberoApi",
        lambda env: FrankaLiberoApi(env, use_sam3=True, use_molmo_fallback=False),
    )
    # SAM3 primary; falls back to Molmo (point seed) -> SAM3 point prompt when
    # SAM3 text segmentation returns no masks. Requires Molmo server when the
    # fallback path is actually triggered.
    register_api(
        "FrankaLiberoApiWithMolmoFallback",
        lambda env: FrankaLiberoApi(env, use_sam3=True, use_molmo_fallback=True),
    )
    # Legacy behaviour: Molmo-first, SAM3 only as point/text backup. Requires
    # the Molmo vLLM service on every call.
    register_api(
        "FrankaLiberoApiMolmoOnly",
        lambda env: FrankaLiberoApi(env, use_sam3=False, use_molmo_fallback=True),
    )
