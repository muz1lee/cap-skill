...
class FrankaLiberoApi(ApiBase):
    ...
    def __init__(
        ...
        self.use_molmo_fallback = use_molmo_fallback
        if self.use_sam3:
        ...
        # Molmo is created lazily on first use; only allowed when use_sam3=False
        # (Molmo is the primary detector) or when use_molmo_fallback=True
        # (SAM3 is primary, Molmo is a backup point seeder).
        self._molmo_point_fn: Any | None = None
    ...
    @property
    def molmo_point_fn(self) -> Any:
        """Lazily-initialized Molmo HTTP client.

        Raises:
            RuntimeError: when Molmo is disabled for this API instance
                (use_sam3=True and use_molmo_fallback=False). Disabled by
                default so SAM3-only configs do not silently depend on the
                Molmo vLLM service.
        """
        if not (self.use_molmo_fallback or not self.use_sam3):
            raise RuntimeError(
                "Molmo is disabled for this FrankaLiberoApi instance "
                "(use_sam3=True, use_molmo_fallback=False). "
                "Use the FrankaLiberoApiWithMolmoFallback or "
                "FrankaLiberoApiMolmoOnly registered API to enable Molmo."
            )
        if self._molmo_point_fn is None:
            self._molmo_point_fn = init_molmo()
        return self._molmo_point_fn
    ...
    def get_object_3d_points_and_masks_from_language(
        ...
        With ``use_sam3=True`` (default), SAM3 text-prompt segmentation is the
        primary path; Molmo is only consulted as a backup point seeder when
        ``use_molmo_fallback=True`` and SAM3 returns nothing. With
        ``use_sam3=False``, the legacy Molmo-first pipeline is used.
        ...
        Returns:
            dict containing:
                - "agentview_mask": np.ndarray of shape (H, W), dtype bool (or None if no mask)
                - "wrist_mask": np.ndarray of shape (H, W), dtype bool (None if single view or no mask)
        ...
        for cam_name in cameras:
            ...
            if self.use_sam3:
                # Primary path: SAM3 text-prompt segmentation.
                masks = self.segment_sam3_text_prompt(rgb, text_prompt)

                if not masks and self.use_molmo_fallback:
                    # Backup: ask Molmo for a seed point, feed it to SAM3
                    # point-prompt. Any Molmo failure (server down, bad parse,
                    # explicit RuntimeError from the lazy property) is degraded
                    # to a warning + empty mask so the trial can continue.
                    point: tuple[int | None, int | None] = (None, None)
                    try:
                        molmo_points = self.point_prompt_molmo(rgb, text_prompt)
                        point = molmo_points.get(text_prompt, (None, None))
                    except Exception as exc:  # noqa: BLE001 - degraded path, see user_rules
                        print(
                            f"[get_object_3d_points_and_masks_from_language] "
                            f"Molmo fallback failed for '{text_prompt}' on {cam_name}: {exc}"
                        )
                ...
                if masks:
                    ...
                    print(
                        f"[get_object_3d_points_and_masks_from_language] "
                        f"No mask for '{text_prompt}' on {cam_name}; "
                        f"returning empty 3D points for this view."
                    )
            else:
                # Legacy path: Molmo first, SAM3 point/text as backup.
                molmo_points = self.point_prompt_molmo(rgb, text_prompt)
                point = molmo_points[text_prompt]
