...
# The unified LLM proxy (capx/serving/llm_proxy_server.py) listens on the same
# port and handles both OpenRouter and the Qwen DashScope proxy, so Qwen
# models share the same URL constant.
_LOCAL_PROXY_URL = OPENROUTER_SERVER_URL

# Qwen proxy (DashScope backend). The local proxy injects
# ``backend=dashscope`` automatically; callers only need to supply the model
# name. See capx/serving/llm_routes.yaml for the upstream definition.
...
def normalize_reasoning_effort(effort) -> tuple[bool, str]:
    ...
    Robust to ``None``, arbitrary case, surrounding whitespace, and
    non-string types (e.g. ``bool``, ``int``).  Raises ``ValueError`` on
    unrecognized tokens so experiments never silently run with the wrong
    thinking mode.
    """
    if effort is None:
        return False, "off"
...
def is_local_proxy_model(model: str) -> bool:
    ...
    The local proxy (``capx/serving/llm_proxy_server.py``) dispatches to
    OpenRouter or the Qwen DashScope backend based on the model name.
    """
...
def query_model(args: "LaunchArgs | ModelQueryArgs", prompt: list[dict]) -> str:
    ...
    # Route models handled by the local proxy (OpenRouter + Qwen DashScope)
    # to the proxy URL; everything else uses the caller-supplied server URL.
...
def query_model_streaming(
    ...
    # Route local-proxy models (OpenRouter + Qwen DashScope) to the unified
    # proxy URL, mirroring the non-streaming path in query_model above.
    if is_local_proxy_model(args.model):
        server_url = _LOCAL_PROXY_URL
    else:
        server_url = args.server_url
    ...
    with requests.post(
        ...
        if is_json and not is_sse:
            ...
            try:
                ...
                _msg = body.get("choices", [{}])[0].get("message") or {}
                full_reasoning = _msg.get("reasoning") or _msg.get("reasoning_content")
                if full_reasoning:
