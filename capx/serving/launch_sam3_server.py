...
def main(
    ...
    # Resolve checkpoint source. Priority: CLI/YAML arg > env var > HF default.
    resolved_ckpt = checkpoint_path or os.environ.get("SAM3_CHECKPOINT_PATH")
        ...
        if resolved_ckpt is not None:
            if not os.path.isfile(resolved_ckpt):
                raise FileNotFoundError(
                    f"SAM3 checkpoint not found at {resolved_ckpt}"
                )
            logger.info(f"Loading SAM3 from local checkpoint: {resolved_ckpt}")
            ...
            logger.info("Loading SAM3 from HuggingFace (facebook/sam3)")
