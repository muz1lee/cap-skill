...
def _load_config(args: LaunchArgs) -> tuple[Any, dict[str, Any], list]:
    ...
    merged_config = {
        ...
        else configs_dict.get("exp_name", None),
        "timestamp": bool(getattr(args, "timestamp", False))
        or bool(configs_dict.get("timestamp", False)),
...
def _save_trial_artifacts(
    ...
    # Structured per-trial metadata (complements the human-readable summary.txt).
    trial_meta = {
        "trial": trial,
        ...
        "has_raw_code": raw_code is not None,
        "has_ensemble": ensemble_data is not None,
        "has_multiturn_ensemble": multiturn_ensemble_data is not None,
        "num_visual_feedback_imgs": len(visual_feedback_imgs),
    }
    (trial_dir / "metadata.json").write_text(json.dumps(trial_meta, indent=2))
...
def _print_and_save_summary(
    ...
    if config["output_dir"]:
        ...
        # Structured run-level metadata for downstream analysis (pandas.json_normalize, etc.).
        from datetime import datetime

        metadata = {
            "run_id": config.get("run_timestamp") or Path(config["output_dir"]).name,
            ...
            "visual_differencing_model": (
                args.visual_differencing_model if config["use_img_differencing"] else None
            ),
            ...
            "agent_flags": {
                "use_img_differencing": config["use_img_differencing"],
                "use_visual_feedback": config["use_visual_feedback"],
                "use_video_differencing": config.get("use_video_differencing", False),
                "use_wrist_camera": config.get("use_wrist_camera", False),
                "use_parallel_ensemble": config["use_parallel_ensemble"],
                "use_multimodel": config["use_multimodel"],
                "use_oracle_code": config["use_oracle_code"],
            },
        }
