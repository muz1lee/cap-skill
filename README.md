...
>
> **Using a local SAM3 checkpoint (offline / mirrored weights):** If you cannot (or do not want to) download from HuggingFace, point the server to a local `sam3.pt` file (the runtime only needs `sam3.pt`; `config.json` is not used). Priority: YAML field > env var > HF default.
>
> - Option A — environment variable (applies to both manual launch and auto-launch during eval):
>
>   ```bash
>   export SAM3_CHECKPOINT_PATH=/path/to/sam3.pt
>   ```
>
> - Option B — per-config YAML field, under the matching `api_servers` entry:
>
>   ```yaml
>   api_servers:
...
>   ```
>
> If the specified file does not exist, the server fails fast instead of silently falling back to HuggingFace.
